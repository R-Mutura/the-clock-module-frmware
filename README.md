...
LONG PRESSING (1second press) the logo button makes the board to go to time setting mode which allows one to set the time.
SINGLE PRESS (press then release) increments the variable you wamt to set by one untill the maximum and then it goes back to zero and repeats
DOUBLE PRESS (pressing two times in relatively fast succession) allows one to change from one variable to the other(from sec to Min.......)
LONG PRESSING AGAIN - exits the time setting area
